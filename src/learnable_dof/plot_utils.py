...
from learnable_dof.dof_learning_utils import SwissRoll, TSNEResult
...
def plot_tsne_result(
    ...
    *,
    ...
    ax_kl.grid(visible=True, alpha=0.2, color=grid_color)
    ...
    # Ensure some vertical margin
    ymin, ymax = min(data.im_alphas), max(data.im_alphas)
    delta = ymax - ymin
    if delta == 0:
        delta = 0.01 * ymax if ymax != 0 else 1.0  # fallback for perfectly flat case

    margin = 0.05 * delta
    ax_alpha.set_ylim(ymin - margin, ymax + margin)

    # Place text slightly below the line, within current limits
    text_y = max(ymin, last_alpha - 0.5 * margin)
    ax_alpha.text(
    ...
    ax_alpha.grid(visible=True, alpha=0.2, color=grid_color)
    ...
    ax_grad.grid(visible=True, alpha=0.2, color=grid_color)
    ...
    fig.suptitle(
        fig_title, fontfamily="DejaVu Sans Mono", fontsize=14, color=text_color, y=0.98
    )
...
def plot_tsne_comparison(
    ...
    *,
    ...
    def add_embedding(
        data: TSNEResult, col_start: int, row: int, labels_array: np.ndarray
    ) -> plt.Axes:
    ...
    ax_kl1 = add_metric_plot(
        0, 1, data1.im_KLs, "min L", "dodgerblue", data1.im_KLs[-1], "orange"
    )
    ax_kl1.set_title(
    ...
    ax_alpha1 = add_metric_plot(
        1, 1, data1.im_alphas, "Converged α", "red", data1.im_alphas[-1], "lime"
    )
    ax_alpha1.set_title(
    ...
    ax_grad1 = add_metric_plot(
        2, 1, data1.im_alpha_grads, "Zero Gradient", "lime", 0, "red"
    )
    ax_grad1.set_title(
    ...
    ax_kl2 = add_metric_plot(
        3, 1, data2.im_KLs, "min L", "dodgerblue", data2.im_KLs[-1], "orange"
    )
    ax_kl2.set_title(
        "KL Divergence", fontfamily="monospace", fontsize=16, color=text_color, pad=10
    )
    ...
    ax_alpha2 = add_metric_plot(
        4, 1, data2.im_alphas, "Converged α", "red", data2.im_alphas[-1], "lime"
    )
    ax_alpha2.set_title(
        "Alpha Values", fontfamily="monospace", fontsize=16, color=text_color, pad=10
    )
    ...
    ax_grad2 = add_metric_plot(
        5, 1, data2.im_alpha_grads, "Zero Gradient", "lime", 0, "red"
    )
    ax_grad2.set_title(
        "Alpha Gradient", fontfamily="monospace", fontsize=16, color=text_color, pad=10
    )
...
def plot_swiss_roll(
    ...
    """
    sr_points = data.datapoints
    sr_color = data.labels
    ...
    if fig is None and ax is None:
        new_fig = plt.figure(figsize=(width, height))
        new_ax = new_fig.add_subplot(111, projection="3d")
    elif ax is None and fig:
        # If figure exists but no axis, create axis at specified position
        new_ax = fig.add_subplot(row, col, (row - 1) * col + col, projection="3d")

    ax = ax if ax is not None else new_ax
    fig = fig if fig is not None else new_fig
    ...
    ax.grid(visible=True, alpha=0.2, color="gray")
