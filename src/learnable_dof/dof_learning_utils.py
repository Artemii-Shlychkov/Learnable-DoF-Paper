...
from openTSNE.affinity import PerplexityBasedNN, PrecomputedAffinities, Uniform
...
def perepare_initial_embedding(
    ...
    n_components: int,
    random_state: int,
    affinities_type: str = "perplexity",
    pre_computed_affinities: PrecomputedAffinities | None = None,
) -> TSNEEmbedding:
    ...
    """
    if not pre_computed_affinities:
        if affinities_type == "uniform":
            affinities_obj = Uniform(
                data=data,
                metric="euclidean",
            )
        elif affinities_type == "perplexity":
        ...
        affinities_obj = pre_computed_affinities
...
def run_tsne(
    ...
    *,
    ...
    initial_embedding = perepare_initial_embedding(
        ...
        pre_computed_affinities=pre_computed_affinities,
    )
